...
! function($) {"use strict";// jshint ;_;
	...
	Carousel.prototype = {
		...
		slide : function(type, next) {
			var $active = this.$element.find('.item.active'), $next = next || $active[type](), isCycling = this.interval, direction = type == 'next' ? 'right' : 'left', fallback = type == 'next' ? 'first' : 'last', that = this, e
...
! function($) {"use strict";// jshint ;_;
	...
	Collapse.prototype = {
		...
		reset : function(size) {
			...
			[dimension](size || 'auto')[0].offsetWidth
...
! function($) {"use strict";// jshint ;_;
	...
	Dropdown.prototype = {
		...
		keydown : function(e) {
			...
			if (!$items.length)
				return index = $items.index($items.filter(':focus'))
	...
	Tab.prototype = {
		...
		show : function() {
			...
			if ($this.parent('li').hasClass('active'))
				return previous = $ul.find('.active:last a')[0]
...
! function($) {"use strict";// jshint ;_;
	...
	Typeahead.prototype = {
		...
		show : function() {
			...
			this.$menu.css({
				top : pos.top + pos.height,
				right : $(window).width() - pos.left - pos.width
			})
